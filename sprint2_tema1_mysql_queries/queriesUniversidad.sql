...
/*
NO FUNCIONA :
SELECT * FROM persona p JOIN (SELECT a.nombre AS asignatura, ce.anyo_inicio AS inicio, ce.anyo_fin AS final FROM asignatura a JOIN alumno_se_matricula_asignatura ama ON a.id = ama.id_asignatura JOIN curso_escolar ce ON ama.id_curso_escolar = ce.id) ON p.id = ama.id_alumno WHERE p.nif LIKE "26902806M";
ESTO OK (SUBQUERY)
SELECT ama.id_alumno, a.nombre AS asignatura, ce.anyo_inicio AS inicio, ce.anyo_fin AS final FROM asignatura a JOIN alumno_se_matricula_asignatura ama ON a.id = ama.id_asignatura JOIN curso_escolar ce ON ama.id_curso_escolar = ce.id;
*/
...
SELECT * FROM persona p JOIN profesor pro ON p.id = pro.id_profesor WHERE pro.id_departamento IS NULL;
